...
<script>
    ...
    variantInput.addEventListener('input', handleSearchInput);
    ...
    // API Functions
    function fetchVariants(searchQuery = '', page = 1) {
        api.get(`/variants?per_page=${per_page}&search=${searchQuery}&page=${page}`)
            .then(response => {
                data_variant = response.data;
                renderVariantTable(data_variant);
                updatePaginationInfo(data_variant.data);
            })
            .catch(error => {
                console.error('Gagal mengambil data variant:', error);
            });
    }
    ...
    function handleSearchInput() {
        ...
        timeout = setTimeout(() => {
            if (query.length > 0) {
                api.get(`/variants?search=${query}&per_page=${per_page}`)
                    .then(response => {
                        temporaryData = response.data;
                        renderVariantTable(temporaryData);
                        updatePaginationInfo(temporaryData.data);
                    })
                    .catch(error => {
                        console.error('Gagal mencari data variant:', error);
                    });
            } else {
                renderVariantTable(data_variant);
                updatePaginationInfo(data_variant.data);
    ...
    function handleCreateForm(e) {
        e.preventDefault();
        showModal('add', 'create-variant-form', `${API_BASE_URL}/variants`, 'post');
    }

    function handleEditForm(e) {
        e.preventDefault();
        document.getElementById('editModal').classList.add('hidden');
        showModal('save', 'edit-variant-form', `${API_BASE_URL}/variants/${selectedId}`, 'put');
    }
    ...
    function createActionCell(item) {
        ...
        `;
        deleteBtn.onclick = () => showModal("delete", "delete-form-" + item.id, `${API_BASE_URL}/variants/${item.id}`, 'delete');
    ...
    function getDataPage(page) {
        if (query.length > 0) {
            api.get(`/variants?search=${query}&per_page=${per_page}&page=${page}`)
                .then(response => {
                    temporaryData = response.data;
                    renderVariantTable(temporaryData);
                    updatePaginationInfo(temporaryData.data);
